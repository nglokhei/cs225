...
class KDTree
{
  ...
  private:
     ...
     */
    void buildTree(int start, int end, int dim);
    int d(const Point<Dim>& first, const Point<Dim>& second) const;
    void quickSort(int start, int end, int dim, int n);
    void swap(int idx1, int idx2);
    void findNearestNeighbor(const Point<Dim>& query, int dim, unsigned int start, unsigned int end, 
