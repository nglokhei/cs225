                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (smallest -> freq.getFrequency() < tmp -> freq.getFrequency()){
	singleQueue.pop();
	return smallest;
    }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
	...
	test1 = removeSmallest(singleQueue, mergeQueue);
	test2 = removeSmallest(singleQueue, mergeQueue);

	TreeNode* node = new TreeNode(test1 -> freq.getFrequency() + test2 -> freq.getFrequency());
	node -> left = test1;
	node -> right = test2;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.hasBits()){
	if (bfile.getNextBit())
	    retval = new TreeNode(Frequency(bfile.getNextByte(), 0));
	else{
